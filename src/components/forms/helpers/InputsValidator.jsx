...
const validateName = name => {
  let regex = new RegExp('^(?=[a-zA-Z0-9._ ]{10,35}$)(?!.*[_.]{2})[^_.].*[^_.]$')
  return regex.test(name)
}

//validate password
const validatePassword = password => {
  let regex = new RegExp('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])(?=.{12,})')
...
export const InputsValidator = ({ nameInput = null, passwordInput = null }) => {
  ...
  if (passwordInput)
    return (
      <div className={`${validatePassword(passwordInput) ? 'text-green-600' : 'text-red-600'} text-xs md:text-sm font-semibold px-2 my-4`}>
        <p>Password Must:-</p>
